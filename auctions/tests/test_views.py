...
class BidViewTest(TestCase):
    def setUp(self):
        self.client = Client()
        self.user = UserFactory()
        self.listing = ListingFactory(starting_bid=Decimal("10.00"), current_bid=Decimal("10.00"))
        self.client.login(username=self.user.username, password='testpassword')

    def test_bid_view_post(self):
        response = self.client.post(reverse('bid', args=[self.listing.id]), {'amount': '12.00'})
        self.assertEqual(response.status_code, 302)
...
class WatchlistViewTest(TestCase):
    ...
    def test_watchlist_add(self):
        ...
        self.assertEqual(response.status_code, 302)
        self.assertRedirects(response, reverse('listing', args=[self.listing.id]))
        self.assertTrue(self.user.watchlist.filter(listing=self.listing, active=True).exists())
...
class CommentViewTest(TestCase):
    ...
    def test_comment_view_post(self):
        response = self.client.post(reverse('comment', args=[self.listing.id]), {'text': 'Test comment'})
        self.assertEqual(response.status_code, 302)
        self.assertRedirects(response, reverse('listing', args=[self.listing.id]))
